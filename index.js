...
const GetNewFactHandler = {
  canHandle(handlerInput) {
    const request = handlerInput.requestEnvelope.request; //intent identified
    return request.type === 'LaunchRequest' // intent major type
      || (request.type === 'IntentRequest' 
        && request.intent.name === 'GetNewFactIntent'); // intent name
  },
  
  handle(handlerInput) {
    const factArr = data; // data set - phrases to return
    const factIndex = Math.floor(Math.random() * factArr.length); // randomiser for phrases to return
    const randomFact = factArr[factIndex]; // randomizer
    const speechOutput = GET_FACT_MESSAGE + randomFact; //output variable

    return handlerInput.responseBuilder //output 
      .speak(speechOutput)
